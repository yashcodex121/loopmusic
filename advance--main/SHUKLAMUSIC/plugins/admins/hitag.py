...
async def mentionall(client, message):
    ...
    try:
        ...
        if participant.status in (
            ...
            is_admin = True
    if not is_admin:
        return await message.reply(f"{ke(_KE_BLOCK,'🚫')} <b>ʏᴏᴜ ᴀʀᴇ ɴᴏᴛ ᴀᴅᴍɪɴ ʙᴀʙʏ, ᴏɴʟʏ ᴀᴅᴍɪɴs ᴄᴀɴ ᴛᴀɢ ᴍᴇᴍʙᴇʀs</b>")

    if message.reply_to_message and message.text:
        return await message.reply(f"{ke(_KE_CROWN,'👑')} <b>ᴜsᴀɢᴇ:</b> /hitag ᴛᴇxᴛ — ᴏʀ ʀᴇᴘʟʏ ᴀɴʏ ᴍᴇssᴀɢᴇ")
    elif message.text:
        mode = "text_on_cmd"
        msg = message.text
    ...
    if chat_id in spam_chats:
        return await message.reply(f"{ke(_KE_WARN,'⚠️')} <b>ᴘʟᴇᴀsᴇ sᴛᴏᴘ ᴄᴜʀʀᴇɴᴛ ᴘʀᴏᴄᴇss ғɪʀsᴛ...</b>")
...
async def cancel_spam(client, message):
    if not message.chat.id in spam_chats:
        return await message.reply(f"{ke(_KE_WARN,'⚠️')} <b>ᴄᴜʀʀᴇɴᴛʟʏ ɪ'ᴍ ɴᴏᴛ ᴛᴀɢɢɪɴɢ ʙᴀʙʏ</b>")
    is_admin = False
    try:
    ...
    if not is_admin:
        ...
        try:
            ...
            pass
        return await message.reply(f"{ke(_KE_OK,'✅')} <b>ᴍᴇɴᴛɪᴏɴ ᴘʀᴏᴄᴇss sᴛᴏᴘᴘᴇᴅ</b>")
